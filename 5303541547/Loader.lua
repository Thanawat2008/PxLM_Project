local DiamondLib = loadstring([[
do
    local DiamondLib = game:GetService("CoreGui"):FindFirstChild("Diamond [Atomic Lib]")
    if DiamondLib then
        DiamondLib:Destroy()
    end
end
...
Atomlib.Name = "Diamond [Atomic Lib]"
...
function  ems:Toggle(text,de,callback)
    ...
    ImageLabel_Toggle.Image = "http://www.roblox.com/asset/?id=8674471848"
...
local Win = DiamondLib:Win()
local Info = Win:addtap("Info")
local Page1 = Info:addpage()
local Page2 = Info:addpage()

Page1:Ti("User Info")

local Data = game.Players.LocalPlayer.Data

local HpLab = Page1:Label("")
local BTLab = Page1:Label("")
local LevelsLab = Page1:Label("")
local GoldLab = Page1:Label("")
local MeleeLab = Page1:Label("")
local DefenseLab = Page1:Label("")
local SwordLab = Page1:Label("")
local PowerLab = Page1:Label("")
local ExpLab = Page1:Label("")
local SpawnPointLab = Page1:Label("")

while wait() do
    local Levels = Data.Levels.Value
    local Gold = Data.Gold.Value
    local Bounty = game:GetService("Players")["LocalPlayer"].leaderstats.Bounty.Value
    local Melee = Data.Melee.Value
    local Defense = Data.Defense.Value
    local Sword = Data.Sword.Value
    local Power = Data.Power.Value
    local Exp = Data.Exp.Value
    local SpawnPoint = Data.SetSpawn.Value

    HpLab:Change("Health Point (s) : "..tostring(game.Players.LocalPlayer.Character.Humanoid.Health))
    LevelsLab:Change("Level (s) : "..tostring(Levels))
    GoldLab:Change("Gold (s) : "..tostring(Gold))
    MeleeLab:Change("Melee Point (s) : "..tostring(Melee))
    DefenseLab:Change("Defense Point (s) : "..tostring(Defense))
    SwordLab:Change("Sword Point (s) : "..tostring(Sword))
    PowerLab:Change("Power Point (s) : "..tostring(Power))
    ExpLab:Change("Exp (s) : "..tostring(Exp))
    SpawnPointLab:Change("Spawn Point : "..tostring(SpawnPoint))
    BTLab:Change("Bountie (s) : "..tostring(Bounty))
end
